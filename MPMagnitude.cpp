...
MPMagnitude *MPMagnitude::initWithUInt32Value(uint32_t value)
{
	return new MPMagnitude(value);
}

MPMagnitude *MPMagnitude::initWithStolenDigits(MPMagnitude *x)
{
	UInt32Vector *digits = x->digits;
	x->digits = NULL;

	return new MPMagnitude(digits);
}

MPMagnitude *MPMagnitude::initWithZeroDigits(size_t n)
{
	return new MPMagnitude(UInt32Vector::initWithSize(n));
}

MPMagnitude::MPMagnitude(uint32_t value)
{
	digits = UInt32Vector::initWithSize(1);
	digits->setItem(0, value);
}

MPMagnitude::MPMagnitude(UInt32Vector *vector)
{
	digits = vector;
}

MPMagnitude::~MPMagnitude()
{
	if (digits != NULL)
	{
		delete digits;
...
MPMagnitude *MPMagnitude::copy() const
{
	return new MPMagnitude(this->digits->copy());
}
...
void MPMagnitude::addUInt32(uint32_t x)
{
	uint64_t carry = x;
...
void MPMagnitude::subtract(const MPMagnitude *x)
{
	if (this == x)
	{
...
void MPMagnitude::subtractUInt32(uint32_t x)
{
...
void MPMagnitude::multiply(const MPMagnitude *x)
{
	if (this == x)
	{
		MPMagnitude *copy = this->copy();
	...
	MPMagnitude *y = new MPMagnitude(this->digits);
...
void MPMagnitude::multiplyUInt32(uint32_t x)
{
	// Store this instances' value
	MPMagnitude *y = new MPMagnitude(this->digits);
...
MPMagnitude *MPMagnitude::divide(const MPMagnitude *x)
{
	assert(!x->isZero());
	...
	if (this == x)
	{
		...
		return MPMagnitude::initWithUInt32Value(0);
	}
...
MPMagnitude *MPMagnitude::longDivide(const MPMagnitude *divisor)
{
	...
	MPMagnitude *normalizedDividend = new MPMagnitude(this->digits); 
	...
	// D2 through 7
	MPMagnitude *remainder = this->normalizedLongDivide(normalizedDividend,
			normalizedDivisor);
...
uint32_t MPMagnitude::estimateQuotientDigit(const MPMagnitude *u, 
		const MPMagnitude *v)
{
 ...
 */
MPMagnitude *MPMagnitude::normalizedLongDivide(const MPMagnitude *dividend, 
		const MPMagnitude *divisor)
{
	...
	MPMagnitude *remainder = new MPMagnitude(UInt32Vector::initWithSize(divisor->digits->size() + 1));
...
uint32_t MPMagnitude::divideUInt32(uint32_t x)
{
	uint64_t carry = 0;
	uint64_t temp;

	const MPMagnitude *copy = new MPMagnitude(this->digits);
...
bool MPMagnitude::equals(const MPMagnitude *x) const
{
...
bool MPMagnitude::isLessThan(const MPMagnitude *x) const
{
	size_t highest = this->digits->highestNonZeroIndex();
	size_t xHighest = x->digits->highestNonZeroIndex();
...
bool MPMagnitude::isLessThanUInt32(uint32_t x) const
{
